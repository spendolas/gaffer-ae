...
| A4 | Checkbox | 183:1052 | state{checked,unchecked} | native `<input type=checkbox>` (MCP rows) | state→`:checked` | 🔶 ARCHITECTURAL: no checkbox in code — MCP rows use Toggle instead. Design decision, needs user call |
...
| A11 | StatusDot (MCP) | 183:1051 | status{auth,connected,failed} | (no dot — `.mcp-tile` recolors) | status→color | 🔶 ARCHITECTURAL: no dot in code — whole tile recolors; AND "connected" renders blue vs Figma spec green. User call |
