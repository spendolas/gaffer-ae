...
| A14 | EffortSlider (ModelSelect) | 484:30131 | value (dot count = model's efforts) | `.effort-slider` (`.e-dot`/`.e-circ` + overlay `.e-fill`/`.e-thumb`/`.e-knob`) | value→thumb px + on-dots | ✅ FULL-PROPERTY audit (not just fills/size/radius): fills incl. hidden layers ✓; strokes — track has 1px `#121212` INSIDE but **visible:false** (so no border is correct, NOT an omission), others none ✓; effects `[]` everywhere ✓; opacity 1 ✓; blend NORMAL/PASS_THROUGH ✓; layout track HORIZONTAL gap2 SPACE_BETWEEN CENTER pad0 ✓; clipsContent track true (overflow:hidden) / dots false ✓; dot frame 24×28 center ✓; hidden `label` 30132 "Medium" visible:false → omitted ✓. Deltas: (a) knob geometry — Figma right-aligns knob inside fill with pad2 (MAX); code centers a 16px knob in the 18px thumb — same visible spot; (b) intentional structural divergence: Figma flows the handle as a flex slot between dots (dots reflow tighter), code keeps 5 evenly-spaced dots + ABSOLUTE thumb overlay so the drag free-follows + eases-to-dot (flex-insert teleports/can't animate). diff-parity e-fill/e-thumb/e-knob width deltas are the 224px clipped-rect-fill technique ≠ growing-width fill — known, accepted per generic-repro principle. |
